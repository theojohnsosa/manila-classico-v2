...
public class ReservationsData {
    ...
    private static String norm(String s){ return s==null ? "" : s.trim().toLowerCase(Locale.ENGLISH); }
    ...
    public static synchronized boolean addReservation(String fullName, String contact, String service,
    ...
    public static synchronized boolean removeFirstReservation() {
        ...
        RES_KEYS.remove(keyOf(
            ...
            removed.getBarber(), removed.getDate(), removed.getTime()
        ));
        rebuildTableModel();
        return true;
    }
    ...
    private static void sortByDateTime() {
        ...
        RES_LIST.sort((a, b) -> {
            try {
                Date da = sdf.parse(a.getDate() + " " + a.getTime());
                Date db = sdf.parse(b.getDate() + " " + b.getTime());
